...
// for the flow engine's inputs. Rendered (not compute) because B8G8R8A8 render targets are
// universally supported while typed BGRA UAVs are an optional capability. Alpha forcing is
// deliberate: NvFBC's desktop alpha is unspecified. Gamma note: values pass through
// untouched (no degamma), so the engine sees the same encoding the display shows.
...
InterpSidecar::InterpSidecar()
    ...
    , m_sharedOutRtv(NULL), m_sharedOut11(NULL)
    ...
    , m_consecutiveFailures(0), m_enabled(false), m_lastProcessUs(0)
{
    ...
    for (int i = 0; i < 2; i++) { m_flowInput[i] = NULL; m_flowInputRtv[i] = NULL; }
}
...
InterpSidecar::~InterpSidecar() {
    ...
    for (int i = 0; i < 2; i++) {
        if (m_flowInputRtv[i]) m_flowInputRtv[i]->Release();
        if (m_flowInput[i]) m_flowInput[i]->Release();
    }
...
bool InterpSidecar::Setup(IDirect3DDevice9Ex* presentDevice, CaptureRing* ring,
                          int width, int height, LONGLONG freqQpc) {
    m_width = width; m_height = height; m_freqQpc = freqQpc;
    ...
    // A flow engine that cannot start fails Setup, which refuses the mode: warp without flow
    // would be a plain blend under another name.
    if (!m_flow.Setup(m_dev11, m_ctx11, m_width, m_height)) return false;

    m_enabled = true;
    LOG("InterpSidecar initialized - D3D11 sidecar + NVOFA raw flow + warp v1, %dx%d BGRA8 "
        "(alpha forced 1.0)", m_width, m_height);
    return true;
}
...
bool InterpSidecar::CreateConversionPipeline() {
    ...
    // Flow engine input textures, BGRA8. The byte-order expectation is a verify point - if
    // the flow comes out wrong on colored content, switch to R8G8B8A8 here and re-run.
    ...
    for (int i = 0; i < 2; i++) {
        HRESULT hr2 = m_dev11->CreateTexture2D(&td, NULL, &m_flowInput[i]);
        if (FAILED(hr2)) { LOGERR("InterpSidecar: flow input %d failed (0x%08x)", i, hr2); return false; }
        hr2 = m_dev11->CreateRenderTargetView(m_flowInput[i], NULL, &m_flowInputRtv[i]);
        if (FAILED(hr2)) { LOGERR("InterpSidecar: flow input RTV %d failed (0x%08x)", i, hr2); return false; }
    }
    return true;
}

bool InterpSidecar::CreateOutputShare(IDirect3DDevice9Ex* presentDevice) {
    // The warp renders straight into the share, so the share's format is the output
    // precision: try 10-bit first and drop to 8-bit if the driver refuses the
    // reverse-direction 10-bit open.
...
bool InterpSidecar::TryCreateOutputShare(IDirect3DDevice9Ex* presentDevice,
    ...
    // Warp output -> this shared texture -> opened on the D3D9 present device. Cleans up
    // after itself on failure so the caller can retry in another format.
    D3D11_TEXTURE2D_DESC td = {};
    td.Width = m_width; td.Height = m_height; td.MipLevels = 1; td.ArraySize = 1;
    td.Format = fmt11;
    td.SampleDesc.Count = 1;
...
bool InterpSidecar::ConvertSlotToBgra(int ringSlot, int inputIdx) {
    ...
    m_ctx11->OMSetRenderTargets(1, &m_flowInputRtv[inputIdx], NULL);
...
bool InterpSidecar::Interpolate(const FrameBracket& bracket) {
    if (!m_enabled) return false;
    if (!bracket.info.hasBefore || !bracket.info.hasAfter) return false;

    // Stateless per present: convert both bracket frames for the FLOW ENGINE (its inputs
    // must be 8-bit), then flow + warp straight into the shared output and flush the
    // cross-API boundary. The WARP samples the original 10-bit ring aliases directly: the
    // 8-bit hop exists only inside flow estimation, where vector precision hides it, never
