...
// DXGI is not in the project's link line (it's a D3D9 app); pull it in here so the helper is
// self-contained and we don't touch all four build configs' AdditionalDependencies.
...
bool VBlankWaiter::Setup(HMONITOR monitor) {
    ...
    // Match the target display by HMONITOR. D3D9 and DXGI adapter ordinals are not guaranteed to
    // correspond, so we identify the output by its monitor handle (the same HMONITOR the window
    // resolves to via MonitorFromWindow) rather than by ordinal.
    for (UINT a = 0; ; a++) {
        ...
        for (UINT o = 0; ; o++) {
            ...
            if (SUCCEEDED(output->GetDesc(&desc)) && desc.Monitor == monitor) {
                m_output = output;   // retain (do not Release)
                adapter->Release();
                factory->Release();
