...
// bits 0-23, interp flag bit 24 (v2, zero), weight bits 25-28, compositor bits 29-30
// (v2, zero), provenance bits 31-32 (v2, zero), pick code bits 33-35, extension
// schema bits 36-38. The checksum covers exactly that span so the decoder's verify
// never changes when v2 lights the remaining reserved cells up.
static void BuildCells(unsigned int counter, int pickCode, int weightQ, bool cells[/*kCells*/]) {
...
unsigned int FrameMarker::Burn(IDirect3DSurface9* backbuffer, int pickCode, int weightQ) {
    ...
    bool cells[kCells];
    BuildCells(burned, pickCode, weightQ, cells);
