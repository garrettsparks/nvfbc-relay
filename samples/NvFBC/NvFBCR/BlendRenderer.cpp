...
// Vertex layout for the fullscreen quad (position + UV). The quad and shaders are ported
// from the validated pre-ring blend implementation (dev @ cf28f40) unchanged, including the
// absence of a half-texel offset — at 1:1 texture-to-backbuffer sizes with LINEAR filtering
// this cost was accepted by the original validation; revisit only with a measurement.
struct QuadVertex {
    float x, y, z;
    float u, v;
};
