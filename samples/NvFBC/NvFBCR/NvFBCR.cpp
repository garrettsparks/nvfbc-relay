...
IFrameCaptureMode* ParseCaptureMode(const string& modeStr) {
    ...
    LOGERR("Invalid capture mode: '%s' (use 'vsync', 't:60' for temporal, 'b' or 'b:vsync' for vsync blend, 'b:60' for timed blend, or numeric fps like '60')", modeStr.c_str());
    return NULL;
}
...
void ConsoleUserInput(string* framerateStr) {
    ...
    cout << "Capture/Present framerate ('vsync', 't:60' for temporal, 'b' for vsync blend, 'b:60' for timed blend, or fps number, blank for vsync) ? ";
    string cinString;
    getline(cin, cinString);
    if (!cinString.empty())
