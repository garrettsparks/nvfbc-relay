...
IFrameCaptureMode* ParseCaptureMode(const string& modeStr) {
    ...
    LOGERR("Invalid capture mode: '%s' (use 'vsync', 't:60' for temporal, 'b:60' for blend, or numeric fps like '60')", modeStr.c_str());
    return NULL;
}
...
void ConsoleUserInput(string* framerateStr) {
    ...
    cout << "Capture/Present framerate ('vsync', 't:60' for temporal, 'b:60' for blend, or fps number, blank for vsync) ? ";
    string cinString;
    getline(cin, cinString);
    if (!cinString.empty())
